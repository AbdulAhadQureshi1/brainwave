    ...
    def _create_index(self):
        ...
        for p_tuple in index:
            ...
            with open(metadata_path, 'r') as file:
                ...
                label = [line.split(":")[-1].strip() for line in metadata if "CPC" in line][0]
                label = int(label) - 1
                outcomes[label] += 1          
...
def create_dataloader(patients_data, root_dir, batch_size=32, records_per_patient=1, val_split=0.2, predict='cpc'):
...
def create_test_loader(patients_data, root_dir, batch_size=32, records_per_patient=1, predict='cpc'):
    dataset = EEGDatasetWinLazy(patients_data, root_dir, window_size=20, records_per_patient=records_per_patient, predict=predict)
