...
class CNNBiLSTMAcrossChannels(nn.Module):
    def __init__(self):
        super(CNNBiLSTMAcrossChannels, self).__init__()

        # Define the CNN layers
        self.cnn_layers = nn.Sequential(
            nn.Conv1d(in_channels=1000, out_channels=512, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
        ...
        self.bilstm = nn.LSTM(
            ...
            hidden_size=19,  # LSTM hidden size
            num_layers=1,   # Number of LSTM layers
            bidirectional=True,  # Bi-LSTM
            batch_first=True
        )

        self.fc1 = nn.Linear(2 * 19, 64) 
    ...
    def forward(self, x):
        ...
        x = x.view(batch_size * num_windows, num_time_steps, num_channels)
