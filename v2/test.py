...
test_loader = DataLoader(val_subset, batch_size=batch_size, num_workers=4, shuffle=False)
...
with torch.no_grad():
    for inputs, labels in tqdm(test_loader):
        ...
        for true_label, pred_label in zip(labels.cpu().numpy(), (outputs.cpu().numpy() > 0.8).astype(int)):
...
cs = tp / (fp + fn) # Challenge Score

print(f"Test Accuracy: {acc*100:.2f}%")
print(f"Test AUC: {auc:.4f}")
print(f"True Positive Rate (TPR): {tpr:.4f}")
print(f"False Positive Rate (FPR): {fpr:.4f}")
print(f"Challenge score: {cs:.4f}")